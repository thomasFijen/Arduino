...
uint8_t 	 DW1000RangingClass::_numNodes = 0;

//module type (anchor or tag)
int16_t      DW1000RangingClass::_type; // TODO enum??

// addresses of current DW1000
byte DW1000RangingClass::_longAddress[LEN_EUI];
byte DW1000RangingClass::_veryShortAddress;
DW1000Device DW1000RangingClass::_selfNode;
...
*/
void DW1000RangingClass::startAsAnchorCustom(uint8_t veryShortAddress, const byte mode[], const bool randomShortAddress) {
	//save the address
...
/*	if (randomShortAddress) {
	...
	//(device Address, network ID, frequency)
	//DW1000Ranging.configureNetwork((uint16_t)_veryShortAddress, 0xDECA, mode);
	DW1000.newConfiguration();
	DW1000.setDefaults();
...
*/
void DW1000RangingClass::startAsTagCustom(uint8_t veryShortAddress, const byte mode[], uint8_t numNodes) {
	//save the address
	for(int i=0;i<LEN_EUI;i++){
		_longAddress[i] = veryShortAddress;
	}
...
void DW1000RangingClass::initAnchors(){
